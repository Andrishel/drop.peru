...
export default function Navbar({ onOpenYape, darkMode, setDarkMode, cartCount, openCart }) {
  ...
  return (
    <header className="sticky top-0 z-50">
      <div className="bg-zinc-950 text-zinc-200 border-b border-emerald-500/30 text-[10px] sm:text-xs font-black uppercase tracking-widest py-2 px-4 text-center relative overflow-hidden">
        <div className="flex items-center justify-center space-x-2 animate-pulse">
          <span>📦 ENTREGAS EL MISMO DÍA EN PIURA, CASTILLA Y 26 DE OCTUBRE • ENVÍOS A TODO EL PERÚ 🔥</span>
        </div>
      </div>

      <div className={`backdrop-blur-md border-b transition-all duration-300 ${darkMode ? 'bg-black/90 border-zinc-800 text-white shadow-black/40' : 'bg-white/90 border-zinc-200 text-zinc-900 shadow-sm'
        }`}>
        <div className="px-4 sm:px-6 py-3 flex items-center justify-between max-w-7xl mx-auto">
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-3">
            ...
            <button
              ...
              className={`relative inline-flex items-center h-7 w-11 sm:w-12 rounded-full p-1 transition-colors duration-300 border ${darkMode ? 'bg-zinc-900 border-zinc-700 justify-end' : 'bg-zinc-200 border-zinc-300 justify-start'
                }`}
            ...
            <button
              onClick={openCart}
              className={`relative flex items-center justify-center transition-all active:scale-95 w-8 h-8 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 rounded-full border ${darkMode
        ...
        <div className={`md:hidden border-t px-4 py-2 flex justify-around text-[11px] font-black uppercase tracking-widest ${darkMode ? 'border-zinc-900 bg-zinc-950/80 text-zinc-400' : 'border-zinc-200 bg-zinc-100/80 text-zinc-600'
          }`}>
